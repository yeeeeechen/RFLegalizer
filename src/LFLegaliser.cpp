...
bool LFLegaliser::initFromGlobalFile(std::string path){
    ...
    for (int i = 0; i < blockNum; i++){
        ...
        if (intx < 0 || inty < 0 || (intx + w) > this->mCanvasWidth || (inty + h) > this->mCanvasHeight){
            std::cout << "[LFL] Warning: in initfromGlobalFile, module " << name << " exceeds canvas\n";
            if (intx < 0){
                intx = 0;
            }
            if (inty < 0){
            ...
            if (intx + w > this->mCanvasWidth){
                w = this->mCanvasWidth - intx;
            }
            if (inty + h > this->mCanvasHeight){
                h = this->mCanvasHeight - inty;
            }
        }

        if ( type == "FIXED" ) {
            Tessera *newTess = new Tessera(tesseraType::HARD, name, area,
                Cord(intx, inty), w, h);
            fixedTesserae.push_back(newTess);
        }
        else {
            Tessera *newTess = new Tessera(tesseraType::SOFT, name, area,
                Cord(intx, inty), w, h);
            softTesserae.push_back(newTess);
        }
    }
    ...
    for (int i = 0; i < connectionNum; i++){
        ...
        ss.str(input);
        while (ss >> input){
            connectionList[i].modules.push_back(input);
        }

        int weight = std::stoi(connectionList[i].modules.back());
        connectionList[i].modules.pop_back();
        connectionList[i].value = (double) weight;
    }
...
void LFLegaliser::initFromFile(std::string path){
    // format: same as visualiseArtpiece()

    std::ifstream fin(path, std::ifstream::in);
    std::string type, name, s;
    int blockNum, area, llx, lly, w, h;
    int softNum, overlapNum;
    fin >> s >> blockNum;
    fin >> this->mCanvasWidth >> this->mCanvasHeight;
    for (int i = 0; i < blockNum; i++){
        fin >> name >> area >> llx >> lly >> w >> h >> type;
        Tessera* newTess;
        if (type == "HARD_BLOCK"){
            newTess = new Tessera(tesseraType::HARD, name, area, Cord(llx, lly), w, h);
            fixedTesserae.push_back(newTess);
        }
        else {
            newTess = new Tessera(tesseraType::SOFT, name, area, Cord(llx, lly), w, h);
            softTesserae.push_back(newTess);   
        }
        fin >> softNum >> overlapNum;

        for (int j = 0; j < softNum + overlapNum; j++){
            fin >> llx >> lly >> w >> h >> type;
            // Tile* newTile;
            // if (type == "BLOCK"){
            //     newTile = new Tile(tileType::BLOCK, Cord(llx, lly), w, h);
            //     newTess->TileArr.push_back(newTile);
            // }
            // else{
            //     newTile = new Tile(tileType::OVERLAP, Cord(llx, lly), w, h);
            //     newTess->OverlapArr.push_back(newTile);
            // }
        }
    }
    fin.close(); 
...
Tile* LFLegaliser::splitTile(Tile* originalTile, Rectangle newRect){
    ...
    if (gtl::yl(newRect) > gtl::yl(originalRect)){
        ...
        Cord newBlNeighborPoint(newBottomTile->getUpperLeft());
        newBlNeighborPoint.x = newBlNeighborPoint.x - 1; 
        for (Tile* leftNeighbor: leftNeighbors){
            if (leftNeighbor->checkCordInTile(newBlNeighborPoint)){
                newBlPtr = leftNeighbor;
...
double LFLegaliser::calculateHPWL(){
    ...
    for(Connection& connection : connectionList){
        ...
        for (std::string moduleName: connection.modules){
            bool found = false;
            double pinx, piny;
            for(Tessera *t : fixedTesserae){
                if (found) break;
                std::string tName = t->getName();
                if(tName == moduleName){
                    found = true;
                    t->calBBCentre(pinx, piny);
                    if (pinx > xmax){
                        xmax = pinx;
                    }
                    if (pinx < xmin){
            ...
            if (!found){
                std::cout << "[LFL] Warning: In calculateHPWL, module " << moduleName << " not found.\n";
            }
        }

        if (xmax < xmin || ymax < ymin){
            std::cout << "[LFL] Warning: In calculateHPWL, invalid net.\n";
        }
        else {
